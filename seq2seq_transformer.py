...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embed_size, dropout, max_len=5000):
        ...
        """
        den = 1 / (e ^ (2i/embed_size) * (math.log(10000)) )
        den = 1 / (e ^ (math.log(10000^(2i/embed_size))))
        den = 1 / (10000^(2i/embed_size))
        """
...
class Seq2SeqTransformer(nn.Module):
    def __init__(self,
        ...
        self.transformar = Transformer(d_model=embed_size,
                                       nhead=nhead,
    ...
    def forward(self, src, trg, src_mask, tgt_mask, src_padding_mask, tgt_padding_mask,
        ...
        outs = self.transformar(src_emb, tgt_emb, src_mask, tgt_mask,
                                None, src_padding_mask, tgt_padding_mask)
        return self.generator(outs)

    def encode(self, src, src_mask):
        return self.transformar.encoder(self.positional_encoding(
            self.src_tok_emb(src)), src_mask)

    def decode(self, tgt, memory, tgt_mask):
        return self.transformar.decoder(self.positional_encoding(
...
def sequential_transforms(*transformers):
    def func(text_input):
        for transformer in transformers:
            text_input = transformer(text_input)
        return text_input
    return func
...
for ln in [SRC_LANGUAGE, TGT_LANGUAGE]:
    text_transform[ln] = sequential_transforms(token_transform[ln],  # 分词
                                               vocab_transform[ln],  # 转成id
                                               tensor_transform)     # 添加BOS, EOS
...
def train_epoch(model, optimizer):
    ...
    for src, tgt in train_dataloader:
        ...
        # tgt输入不包含 EOS
        tgt_input = tgt[:-1, :]
        src_mask, tgt_mask, src_padding_mask, tgt_padding_mask = creat_mask(src, tgt_input)
...
for epoch in range(1, NUM_EPOCHS+1):
    ...
    print(f"Epoch: {epoch}, Train loss:{train_loss:.3f}, Val loss:{val_loss:.3f}, Epoch time: {(end_time-start):.3f} s")


# save model
torch.save(transformer.state_dict(), "./de_to_en.pth")
